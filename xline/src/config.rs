...
/// General configuration object, , including some basic configuration fields
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq, Eq, Getters)]
pub struct GeneralConfig {
    /// Get xline server name
    #[getset(get = "pub")]
    name: String,
    /// Xline server host addr, including ip address and the port.
    #[getset(get = "pub")]
    host: SocketAddr,
}

impl GeneralConfig {
    /// Generate a new `GeneralConfig` object
    #[must_use]
    #[inline]
    pub fn new(name: String, host: SocketAddr) -> Self {
        Self { name, host }
    }
}
...
impl ClusterConfig {
    ...
    #[must_use]
    #[inline]
    pub fn new(peers: Vec<SocketAddr>, leader: SocketAddr) -> Self {
        Self { peers, leader }
    }
}
...
mod tests {
    ...
    fn test_xline_server_config_shoule_be_loaded() {
        ...
        assert_eq!(
            ...
            GeneralConfig::new(
                "node1".to_owned(),
                SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 2379),
            )
        );
        assert_eq!(
            config.cluster,
            ClusterConfig::new(
                vec![
                    SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 2380),
                    SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 2381),
                ],
                SocketAddr::new(IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)), 2379),
            )
        );
