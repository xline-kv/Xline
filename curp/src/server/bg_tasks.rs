...
>(
    ...
    loop {
        ...
        // send append_entries to each server in parallel
        handle_heartbeat_responses(rpcs, Arc::clone(&state)).await;
    }
}
...
async fn handle_heartbeat_responses<C, S, Conn, ExeTx>(rpcs: S, state: Arc<RwLock<State<C, ExeTx>>>)
where
    C: Command + 'static,
...
mod test {
    ...
    async fn leader_will_calibrate_follower_until_succeed() {
        ...
        leader_calibrates_follower(Arc::new(mock_connect), Arc::clone(&state), 2).await;
        {
            let state = state.read();
    ...
    async fn leader_calibrate_follower_will_calibrate_term() {
        ...
        leader_calibrates_follower(Arc::new(mock_connect), Arc::clone(&state), 0).await;
