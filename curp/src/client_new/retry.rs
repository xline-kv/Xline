...
use super::{ClientApi, ProposeResponse};
...
pub(super) struct RetryConfig {
    ...
    /// Retry count
    count: usize,
    /// Enable jitter to randomize the delay
    jitter: bool,
}
...
/// Update leader state
pub(super) trait LeaderStateUpdate {
    /// update
    fn update_leader(&self, leader_id: Option<ServerId>, term: u64);
}

impl<C, Api> Retry<C, Api>
where
    C: Command,
    Api: ClientApi<C, Error = CurpError> + LeaderStateUpdate + Send + Sync + 'static,
{
...
#[async_trait]
impl<C, Api> ClientApi<C> for Retry<C, Api>
where
    C: Command,
    Api: ClientApi<C, Error = CurpError> + LeaderStateUpdate + Send + Sync + 'static,
{
    ...
    async fn fetch_read_state(&self, cmd: &C) -> Result<ReadState, tonic::Status> {
