...
use xline_client::{error::Result, types::auth::AuthRoleRevokePermissionOption, Client};

use crate::utils::printer::Printer;

/// Temp request type for `revoke_perm` command
type AuthRoleRevokePermissionRequest = (String, Vec<u8>, Option<AuthRoleRevokePermissionOption>);
...
pub(super) fn build_request(matches: &ArgMatches) -> AuthRoleRevokePermissionRequest {
    ...
    if let Some(range_end) = range_end {
        option = Some(AuthRoleRevokePermissionOption::WithRangeEnd(
        ...
        let test_cases = vec![
            TestCase::new(
                ...
                Some((
                    "Admin".into(),
                    "key1".into(),
                    Some(AuthRoleRevokePermissionOption::WithRangeEnd("key2".into())),
                )),
            ),
            TestCase::new(
