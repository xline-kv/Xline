...
#[tokio::test(flavor = "multi_thread")]
#[abort_on_panic]
async fn xline_remove_node_etcd_client() -> Result<(), Box<dyn Error>> {
    let mut cluster = Cluster::new(5).await;
    cluster.start().await;
    let mut cluster_client = EtcdClient::connect(cluster.all_client_addrs(), None)
        .await?
        .cluster_client();
    let list_res = cluster_client.member_list().await?;
    assert_eq!(list_res.members().len(), 5);
    let remove_id = list_res.members()[0].id();
    let remove_res = cluster_client.member_remove(remove_id).await?;
    assert_eq!(remove_res.members().len(), 4);
...
#[tokio::test(flavor = "multi_thread")]
#[abort_on_panic]
async fn xline_add_node_etcd_client() -> Result<(), Box<dyn Error>> {
    let mut cluster = Cluster::new(3).await;
    cluster.start().await;
    let client = EtcdClient::connect(cluster.all_client_addrs(), None).await?;
    let mut cluster_client = client.cluster_client();
    ...
    let new_node_client_listener = TcpListener::bind("0.0.0.0:0").await?;
    let new_node_client_urls = vec![format!("http://{}", new_node_client_listener.local_addr()?)];
    let add_res = cluster_client.member_add(new_node_peer_urls, None).await?;
    assert_eq!(add_res.member_list().len(), 4);
    cluster
        .run_node(new_node_client_listener, new_node_peer_listener)
        .await;
    let mut etcd_client = etcd_client::Client::connect(&new_node_client_urls, None).await?;
...
#[tokio::test(flavor = "multi_thread")]
#[abort_on_panic]
async fn xline_update_node_etcd_client() -> Result<(), Box<dyn Error>> {
    let mut cluster = Cluster::new(3).await;
    cluster.start().await;
    let mut cluster_client = cluster.client().await.cluster_client();
    let old_list_res = cluster_client.member_list(false).await?;
