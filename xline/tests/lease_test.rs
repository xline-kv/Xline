...
async fn test_lease_keep_alive() -> Result<(), Box<dyn Error>> {
    let mut cluster = Cluster::new(3).await;
    cluster.start().await;
    let leader_ep = cluster.addrs()[0].to_string();
    let client = cluster.client().await;
    ...
    // TODO: only leader can handle lease keep alive request now, refactor this test when it's fixed
    let mut leader_client = etcd_client::Client::connect(vec![leader_ep], None).await?;

    let (mut keeper, mut stream) = leader_client.lease_keep_alive(lease_id).await?;
